...
APP_NAME = "KHÁNH IT - AI IMAGE CREATOR V3.2.2"

def get_writable_app_dir():
    """Chọn thư mục chắc chắn có quyền ghi trên Windows."""
    candidates = []

    local_appdata = os.getenv("LOCALAPPDATA")
    if local_appdata:
        candidates.append(Path(local_appdata) / "KHANH_IT_AI_IMAGE_CREATOR")

    user_profile = os.getenv("USERPROFILE")
    if user_profile:
        candidates.append(Path(user_profile) / "Documents" / "KHANH_IT_AI_IMAGE_CREATOR")

    temp_dir = os.getenv("TEMP") or os.getenv("TMP")
    if temp_dir:
        candidates.append(Path(temp_dir) / "KHANH_IT_AI_IMAGE_CREATOR")

    candidates.append(Path.cwd() / "KHANH_IT_AI_IMAGE_CREATOR_DATA")
    ...
    raise PermissionError(
        "Không tìm được thư mục có quyền ghi để lưu dữ liệu phần mềm."
    )

APP_DATA_ROOT = get_writable_app_dir()
OUTPUT_DIR = APP_DATA_ROOT / "generated_images"
DATA_DIR = APP_DATA_ROOT / "app_data"
