...
class PNGConverterTool:
    def __init__(self, root):
        self.root = root
        self.root.title("KHÁNH IT - CHUYỂN ĐỔI ẢNH PNG 16:9")
    ...
    def create_interface(self):
        ...
        tk.Label(
            header_frame,
            text="KHÁNH IT - PNG IMAGE CONVERTER",
            font=("Arial", 20, "bold"),
            fg="#38BDF8",
            bg="#111827"
        ).pack(pady=(14, 2))
        ...
        tk.Label(
            header_frame,
            text="Chuyển ảnh PNG về chuẩn 16:9 • 1920 × 1080",
            font=("Arial", 11),
            fg="white",
            bg="#111827"
        ).pack()
        ...
        tk.Label(
            color_row,
            text="Ví dụ: #000000 là đen, #FFFFFF là trắng",
            font=("Arial", 9),
            fg="#555555"
        ...
        # =================================================
        # NÚT BẮT ĐẦU LUÔN HIỂN THỊ Ở PHÍA DƯỚI
        # =================================================
    ...
    def select_input_folder(self):
        folder = filedialog.askdirectory(
            title="Chọn thư mục chứa ảnh PNG"
        )
    ...
    def update_format_status(self, event=None):
        ...
        if input_folder and os.path.isdir(input_folder):
            current_output = self.output_folder.get().strip()
            ...
            default_jpg = os.path.join(
                input_folder,
                "PNG_TO_JPG_1920X1080"
            )

            if not current_output or current_output in (
                default_png,
                default_jpg
            ):
                if output_format == "JPG":
                    self.output_folder.set(default_jpg)
                else:
                    self.output_folder.set(default_png)
    ...
    def start_processing(self):
        ...
        if not input_folder:
            messagebox.showwarning(
                "Thiếu thư mục",
                "Vui lòng chọn thư mục chứa ảnh PNG."
            )
            return
        ...
        if not os.path.isdir(input_folder):
            messagebox.showerror(
                ...
                "Thư mục chứa ảnh PNG không tồn tại."
            )
            return
        ...
        if not output_folder:
            messagebox.showwarning(
                ...
                "Vui lòng chọn thư mục lưu ảnh."
            )
            return

        try:
            ImageColor.getrgb(
                self.background_color.get().strip()
            )
        except ValueError:
            messagebox.showerror(
                "Màu nền không hợp lệ",
                "Vui lòng nhập mã màu dạng HEX.\n\n"
                "Ví dụ:\n"
                "#000000 là màu đen\n"
                "#FFFFFF là màu trắng"
            )
        ...
        if output_format == "JPG":
            try:
                ...
                messagebox.showerror(
                    "Chất lượng không hợp lệ",
                    "Chất lượng JPG phải từ 50 đến 100."
                )
                return
        ...
        self.status_label.config(
            text="Đang tìm ảnh PNG trong thư mục..."
        )
    ...
    def process_images(self):
        ...
        try:
            ...
            self.root.after(
                0,
                self.write_log,
                f"Tìm thấy {total_files} ảnh PNG."
            )
    ...
    def convert_image(
        ...
        background_rgb = ImageColor.getrgb(
            self.background_color.get().strip()
        )
        ...
        with Image.open(input_path) as original_image:
            ...
            if output_format == "JPG":
                # JPG không hỗ trợ trong suốt.
                # Ghép ảnh PNG lên màu nền trước khi lưu JPG.
                jpg_background = Image.new(
                    "RGB",
                    TARGET_SIZE,
                    background_rgb
                )
    ...
    def create_unique_output_path(
        ...
        if (
            ...
            != os.path.abspath(input_path).lower()
        ):
            return output_path
        ...
        while True:
            new_name = (
                f"{base_name}_1920x1080_{counter}"
                f"{extension}"
            )
    ...
    def no_png_found(self):
        self.status_label.config(
            text="Không tìm thấy ảnh PNG."
        )

        messagebox.showwarning(
            "Không có ảnh PNG",
            "Không tìm thấy file .png trong thư mục đã chọn."
        )
    ...
    def processing_complete(
        ...
        self.write_log(
            f"Hoàn thành: {success_count} ảnh thành công, "
            f"{error_count} ảnh lỗi."
        )

        messagebox.showinfo(
            "Hoàn thành",
            f"Đã xử lý xong ảnh PNG.\n\n"
            f"Định dạng đầu ra: {output_format}\n"
            f"Kích thước: 1920 × 1080\n"
            f"Thành công: {success_count} ảnh\n"
            f"Lỗi: {error_count} ảnh\n\n"
            f"Thư mục lưu:\n{output_folder}"
        )
